...
export function checkWorkZoInterviewAllowed(plan = getWorkZoCurrentPlan()) {
  ...
  return {
    allowed: used < limit,
    reason: used < limit ? "ok" : "freeLimit",
...
export function checkWorkZoTavusAllowed(plan = getWorkZoCurrentPlan()) {
  ...
  if (usage.tavusInterviewsStarted >= limits.tavusInterviewsPerMonth) {
    return {
      allowed: false,
      reason: "video_recruiter_interview_limit",
      remaining: 0,
      limit: limits.tavusInterviewsPerMonth,
      used: usage.tavusInterviewsStarted,
    };
  ...
  if (usage.tavusMinutesUsed >= limits.tavusMinutesPerMonth) {
    return {
      allowed: false,
      reason: "video_recruiter_minutes_limit",
      remaining: 0,
      limit: limits.tavusMinutesPerMonth,
      used: usage.tavusMinutesUsed,
    };
  }
...
export function recordWorkZoInterviewStarted() {
  const next = updateWorkZoUsage((usage) => ({
    ...usage,
    interviewsStarted: usage.interviewsStarted + 1,
  }));
...
export function recordWorkZoReportViewed() {
  const next = updateWorkZoUsage((usage) => ({
    ...usage,
    reportsViewed: usage.reportsViewed + 1,
  }));
...
export function recordWorkZoTavusInterviewStarted() {
  const next = updateWorkZoUsage((usage) => ({
    ...usage,
    tavusInterviewsStarted: usage.tavusInterviewsStarted + 1,
  }));
...
export function recordWorkZoTavusMinutes(minutes: number) {
  const next = updateWorkZoUsage((usage) => ({
    ...usage,
    tavusMinutesUsed: usage.tavusMinutesUsed + Math.max(0, minutes),
  }));
...
export function recordWorkZoUpgradeClick() {
  const next = updateWorkZoUsage((usage) => ({
    ...usage,
    upgradeClicks: usage.upgradeClicks + 1,
  }));
...
export function getWorkZoUsageSummary(plan = getWorkZoCurrentPlan()) {
  ...
  return {
    ...
    limits: {
      ...limits,
      interviewsPerMonth: interviewLimit,
    },
    usage,
    testMode: isWorkZoFounderTestMode(),
    interviewsRemaining: Math.max(0, interviewLimit - usage.interviewsStarted),
    tavusInterviewsRemaining: isWorkZoFounderTestMode()
      ? 999
      : Math.max(0, limits.tavusInterviewsPerMonth - usage.tavusInterviewsStarted),
    tavusMinutesRemaining: isWorkZoFounderTestMode()
      ? 999
      : Math.max(0, limits.tavusMinutesPerMonth - usage.tavusMinutesUsed),
  };
}
