...
import { resolveWorkZoInterviewModel } from "@/lib/workzoModelRouting";
...
export type UnifiedRecruiterInput = {
  ...
  /** Server-resolved plan (never trust a client-supplied value here), drives
   * priority model routing via workzoModelRouting.ts. */
  plan?: string | null;
};
...
`;
  ...
  const _recruiterProfileMap: Record<string, { name: string; role: string; behaviorPrompt: string }> = {
    ...
    faang_hiring_manager: {
      ...
      behaviorPrompt: "You are Alex Chen, a Senior Technical Interviewer. Your interview is grounded in the specific job description first, then the candidate CV. Preserve CV section boundaries: never conflate Projects, Education, and Work Experience, and never attribute a project to an employer unless it is explicitly nested under that employer. Respect chronology and dates. Use at most one specific CV detail to anchor a question. Ask one clear question per turn. If the candidate is vague twice on the same topic, score the gap internally, pivot, and continue. For non-engineering roles, focus on transferable technical thinking only where it supports the target JD.",
    },
    alex: {
      name: "Alex Chen",
      role: "Technical Interviewer",
...
INTERVIEW ORCHESTRATION AUTHORITY:
Any INTERVIEW ORCHESTRATION DIRECTIVES block above is authoritative and overrides every other instinct in this prompt.
Follow-up discipline at all times: a complete answer means move to a new topic; a partial answer earns at most ONE follow-up; a weak or evasive answer earns at most TWO in total, then you move on and silently note the gap for your assessment. Real interviewers never interrogate one story for a third of the session.
Topics marked COVERED are locked: never re-ask them, even rephrased. Prefer the least-covered open topic.
Never begin a reply with an opening phrase listed as already used; vary between walk-throughs, hypotheticals, look-backs, quantification asks, and trade-off probes.
When a CLOSING SEQUENCE step appears, execute exactly that step: no extra questions, no skipping the candidate-questions invitation, and end with a warm goodbye at step 3.
...
3b. Preserve CV section and timeline boundaries. Do not attach a Projects item to a Work Experience employer, and do not attach later education/bootcamp tools to an earlier job, unless explicitly stated. If unclear, ask neutrally instead of assuming.
...
0b. CV SECTION BOUNDARY RULE, Never merge unrelated CV sections into one statement. Projects are projects. Education/bootcamp is education. Employment is employment. Do not say “At [company], you did [project]” unless the CV explicitly places that project under that company. Use maximum one CV detail as an anchor before asking your question.
...
export async function decideUnifiedRecruiterResponse(
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      // Priority AI processing: Premium Pro users get the priority model
      // (OPENAI_INTERVIEW_MODEL_PRO), everyone else the standard one.
      model: resolveWorkZoInterviewModel(input.plan),
      temperature: 0.62,
