...
export type UnifiedRecruiterInput = {
  ...
  setup?: {
    ...
    // HARD constraint block from the Interview Director (interviewDirector.ts).
    // Unlike recruiterBrainContext (advisory), this fixes the NEXT AREA the
    // model must ask about — it picks the wording, not the topic.
    directorConstraint?: string;
...
function buildSystemPrompt(
  ...
  const directorConstraint = cleanText(setup.directorConstraint || "").slice(0, 1800);
