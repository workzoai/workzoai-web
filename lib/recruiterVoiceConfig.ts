...
export const recruiterVoiceProfiles: Record<RecruiterPersonality, RecruiterVoiceProfile> = {
  friendly_hr: {
    key: "friendly_hr",
    name: "Sarah",
    role: "Friendly HR",
    voiceGender: "female",
    assistantId: process.env.NEXT_PUBLIC_VAPI_SARAH_ASSISTANT_ID,
    pacing: "calm",
    interruptionStyle:
  ...
  analytical_hiring_manager: {
    ...
    role: "Hiring Manager",
    voiceGender: "male",
    assistantId: process.env.NEXT_PUBLIC_VAPI_DANIEL_ASSISTANT_ID,
    pacing: "balanced",
    interruptionStyle:
  ...
  startup_recruiter: {
    ...
    voiceGender: "female",
    assistantId: process.env.NEXT_PUBLIC_VAPI_PRIYA_ASSISTANT_ID,
    pacing: "fast",
    interruptionStyle:
  ...
  corporate_recruiter: {
    ...
    voiceGender: "male",
    assistantId: process.env.NEXT_PUBLIC_VAPI_MARKUS_ASSISTANT_ID,
    pacing: "structured",
    interruptionStyle:
...
export function normalizeRecruiterPersonality(value?: string): RecruiterPersonality {
  if (
    value === "friendly_hr" ||
...
export function getVapiAssistantIdForRecruiter(value?: string): string | undefined {
  ...
  return profile.assistantId || process.env.NEXT_PUBLIC_VAPI_ASSISTANT_ID;
}
