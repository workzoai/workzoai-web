        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-xs font-black uppercase tracking-[0.2em] text-brand">WorkZo Talent Intelligence</p>
              <span className={cx("rounded-full border px-2 py-0.5 text-[10px] font-black uppercase tracking-wide", isLive ? "border-success/30 bg-success/10 text-success" : "border-warning/30 bg-warning/10 text-warning")}>{isLive ? `Live · ${data.org}` : "Sample B2B portal"}</span>
            ...
            <p className="mt-1 max-w-3xl text-sm text-muted">Cohort heatmaps, rejection-risk signals, curriculum actions, and opt-in employer-ready talent pipeline in one dashboard.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {isLive ? <button onClick={() => { const p = new URLSearchParams(window.location.search); void loadOrg(p.get("org") || p.get("code") || "", p.get("key") || "", p.get("secret") || ""); }} className="inline-flex items-center gap-2 rounded-xl border border-line bg-fg/[0.04] px-5 py-3 text-sm font-black text-fg hover:bg-fg/[0.08]"><RefreshCw className={cx("h-4 w-4", loading && "animate-spin")} /> Refresh</button> : <a href="mailto:support@workzoai.com?subject=WorkZo%20AI%20Talent%20Pipeline%20Pilot" className="inline-flex items-center gap-2 rounded-xl bg-brand px-5 py-3 text-sm font-black text-on-brand shadow-[0_8px_24px_-8px_rgba(37,99,235,0.6)] hover:bg-brand-strong">Request B2B pilot <ArrowUpRight className="h-4 w-4" /></a>}
          </div>
        </header>
