...
export default function OnboardingPage() {
  ...
  // IMPORTANT:
  // Do NOT block the whole onboarding page while /api/account/plan is loading.
  // That endpoint depends on auth + subscription lookup and can be slow/hang.
  // If we wait for planState.loading === false before rendering, users can see
  // a blank spinner forever and never reach CV upload.
  //
  // Safe behavior:
  // - While plan is still loading: render onboarding normally using free/default limits.
  // - If auth finishes and user is unauthenticated: redirect to login.
  // - Never let plan lookup block the first onboarding experience.
  if (!planState.loading && !planState.authenticated) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#020712] text-white">
        <div className="text-center">
          <div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-white/20 border-t-white/80" />
          <p className="mt-4 text-sm text-white/60">Redirecting you to sign in...</p>
        </div>
      </main>
    );
  }
