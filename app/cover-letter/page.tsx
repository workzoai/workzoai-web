  ...
  return (
    <PremiumFeatureGate feature="cover_letter" title="Cover Letter is a Premium feature" description="Role-specific cover letters based on your CV and job description are included in Premium.">
      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3">
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-lg border border-white/10 bg-white/[0.04] p-6">
            ...
            <div className="mt-6 rounded-xl border border-cyan-300/15 bg-cyan-400/[0.045] p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-lg border border-white/10 bg-black/20 p-3"><Gauge className="mb-2 h-4 w-4 text-blue-200" /><p className="text-xs text-slate-400">Current chance</p><p className="text-xl font-black">{phaseA.interviewProbability.current}%</p></div>
                <div className="rounded-lg border border-white/10 bg-black/20 p-3"><Target className="mb-2 h-4 w-4 text-blue-200" /><p className="text-xs text-slate-400">After CV fix</p><p className="text-xl font-black">{phaseA.interviewProbability.afterCvFix}%</p></div>
                <div className="rounded-lg border border-white/10 bg-black/20 p-3"><FileText className="mb-2 h-4 w-4 text-blue-200" /><p className="text-xs text-slate-400">After prep</p><p className="text-xl font-black">{phaseA.interviewProbability.afterInterviewPrep}%</p></div>
              </div>
            </div>


            <div className="mt-6 rounded-xl border border-blue-300/15 bg-blue-500/[0.045] p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
            ...
            <div className="mt-6 space-y-4">
              <label className="block">
                ...
                <input value={targetRole} onChange={(e) => setTargetRole(e.target.value)} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm outline-none focus:border-blue-400" />
              </label>
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase tracking-[.18em] text-slate-400">CV context</span>
                <textarea value={cvText} onChange={(e) => setCvText(e.target.value)} rows={8} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm outline-none focus:border-blue-400" />
              </label>
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase tracking-[.18em] text-slate-400">Job description</span>
                <textarea value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={8} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm outline-none focus:border-blue-400" />
              </label>
              <button onClick={handleGenerate} disabled={loading} className="inline-flex items-center gap-2 rounded-lg bg-blue-500 px-5 py-3 text-sm font-black text-white hover:bg-blue-400 disabled:opacity-60">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.035] p-6">
            ...
            <pre className="mt-5 min-h-[520px] whitespace-pre-wrap rounded-lg bg-black/20 p-5 text-sm leading-7 text-slate-200">
