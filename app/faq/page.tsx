...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-line last:border-b-0">
      ...
      >
        <span className="text-base font-black text-fg leading-6">{q}</span>
        <ChevronDown className={`mt-0.5 h-5 w-5 shrink-0 text-muted transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>
  ...
  return (
    <main className="min-h-screen bg-canvas text-fg">
      ...
      <div className="mx-auto max-w-4xl px-5 py-12 sm:px-6 lg:px-8">
            ...
            <button
              ...
              className={`rounded-full border px-4 py-1.5 text-sm font-black transition ${
                active === c.id
                  ...
                  : "border-line bg-fg/[0.04] text-muted hover:border-line hover:text-fg"
              }`}
            >
              {c.label}
            </button>
        ...
        <div className="mt-6 rounded-lg border border-line bg-fg/[0.03] px-6">
          {visible.map((f) => <FaqItem key={f.q} {...f} />)}
        </div>

        <div className="mt-10 rounded-lg border border-brand/20 bg-brand/[0.07] p-7">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <a href="mailto:support@workzoai.com" className="inline-flex items-center gap-2 rounded-lg bg-brand px-5 py-3 text-sm font-black text-on-brand hover:bg-brand">
              support@workzoai.com <ArrowRight className="h-4 w-4" />
            </a>
            <Link href="/help" className="inline-flex items-center gap-2 rounded-lg border border-line bg-fg/[0.04] px-5 py-3 text-sm font-black text-fg hover:bg-fg/10">
              Help center
            </Link>
          </div>
        </div>
