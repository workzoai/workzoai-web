    ...
    <main className="min-h-screen bg-canvas px-5 py-8 text-fg">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-10 rounded-lg border border-line bg-gradient-to-br from-brand/15 via-brand/10 to-white/[0.03] p-7 sm:p-10">
          ...
          <p className="mt-5 max-w-3xl text-base leading-8 text-muted sm:text-lg">{feature.description}</p>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="/pricing?intent=upgrade" className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand px-6 py-4 text-sm font-black text-on-brand hover:bg-brand">
              {feature.cta}
              <ArrowRight className="h-4 w-4" />
            </Link>
            <Link href="/pricing?intent=interview" className="inline-flex items-center justify-center gap-2 rounded-lg border border-line px-6 py-4 text-sm font-black text-fg hover:bg-fg/10">
              Try free interview
            </Link>
          </div>
        </section>
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-2">
          <div className="rounded-lg border border-success/20 bg-success/[0.06] p-6">
            <p className="text-sm font-black uppercase tracking-[0.22em] text-success">Free preview</p>
            <h2 className="mt-3 text-2xl font-black">What users can see before upgrading</h2>
            <div className="mt-5 space-y-3">
              {feature.freePreview.map((item) => (
                <div key={item} className="flex gap-3 text-sm leading-6 text-fg">
        ...
        <section className="mt-6 rounded-lg border border-line bg-fg/[0.04] p-6">
          <div className="flex items-start gap-3">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-warning/10 text-warning">
              <ShieldCheck className="h-5 w-5" />
            </div>
            <div>
              <p className="text-sm font-black uppercase tracking-[0.22em] text-muted">Example</p>
              <p className="mt-3 max-w-3xl text-base leading-8 text-fg">{feature.example}</p>
            </div>
          </div>
        </section>
      </div>
