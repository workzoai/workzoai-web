...
import {
  computeCvCompletenessScore,
  type WorkZoAccountScores,
} from "@/lib/workzoCvScore";
import { buildReadinessScore } from "@/lib/workzoReadinessEngine";
...
const actionCards = [
  ...
  },
  {
    ...
    detail: "Check LinkedIn against your CV and the job",
    href: "/linkedin",
    icon: UserRound,
    feature: "linkedin_analyze" as const,
    required: "free" as WorkZoPlanType,
    accent: "text-brand",
    bg: "bg-brand/10",
  },
...
const proTools = [
  {
    ...
    detail: "Personal coaching priorities from your history",
    icon: BrainCircuit,
    feature: "career_coach" as const,
  },
  {
    title: "Live AI Recruiter",
    detail: "60 AI video minutes per month",
    icon: Video,
    feature: "video_recruiter" as const,
  },
  {
    title: "Career Roadmaps",
    detail: "30 / 60 / 90 day improvement plans",
    icon: TrendingUp,
    feature: "career_roadmaps" as const,
  },
  {
    title: "Replay Intelligence",
    detail: "Best moments, weak moments, and missed opportunities",
    icon: PlayCircle,
    feature: "replay_intelligence" as const,
  },
];
...
function freeToolNavItem(id: string, label: string): SidebarNavItem {
  ...
  return {
    label,
    href: `/tools/${id}`,
    icon: tool ? getFreeToolIcon(tool.icon) : Sparkles,
  };
}
...
const navItems: SidebarNavItem[] = [
  ...
  freeToolNavItem("cv-review", "CV Review"),
  { label: "LinkedIn Optimizer", href: "/linkedin", icon: UserRound },
  { label: "Cover Letter", href: "/cover-letter", icon: Mail },
  freeToolNavItem("interview-questions", "Interview Questions"),
...
export default function DashboardPage() {
  ...
  const [usage, setUsage] = useState<ReturnType<
    typeof getWorkZoUsageSummary
  > | null>(null);
  const [accountScores, setAccountScores] =
    useState<WorkZoAccountScores | null>(null);
  const [cvScoreInfo, setCvScoreInfo] =
    useState<ReturnType<typeof computeCvCompletenessScore>>(null);
  useEffect(() => {
    let active = true;
        ...
        if (serverUsage) {
          ...
          setUsage({
            ...
            interviewsRemaining: Number(
              serverUsage.interviewsRemaining ??
                localSummary.interviewsRemaining,
            ),
          });
        }
      })
    ...
    try {
      ...
      for (const k of keys) {
        ...
        if (raw) {
          ...
          const name =
            parsed?.candidateName || parsed?.state?.candidateName || "";
          const first = name.split(" ")[0];
          if (
            first &&
            first.length > 1 &&
            !/unknown|section|header/i.test(first)
          ) {
            setFirstName(first);
            break;
          }
  ...
  // Real score data: interview average from the DB, CV score computed
  // structurally (JD-less) from whatever CV is in this session, and a
  // composite readiness score built from whichever of those are available.
  // Runs independently of the plan fetch above so a slow /api/account/plan
  // call never blocks these from loading.
  useEffect(() => {
    let active = true;
    ...
    fetch("/api/account/scores", { cache: "no-store", credentials: "include" })
      ...
      .then((data: WorkZoAccountScores | null) => {
        if (active && data) setAccountScores(data);
      })
      .catch(() => undefined);
    ...
    try {
      ...
      const profile =
        (setup?.resumeProfile as ResumeProfile | undefined) || null;
      setCvScoreInfo(computeCvCompletenessScore(profile));
    } catch {
      setCvScoreInfo(null);
    }
  ...
  const readiness = useMemo(() => {
    ...
    try {
      const raw =
        typeof window !== "undefined"
          ? window.localStorage.getItem("workzo_technical_result")
          : null;
    ...
    const hasAnySignal =
      Boolean(cvScoreInfo) ||
      Boolean(accountScores?.avgInterviewScore) ||
      Boolean(technicalResult);
    if (!hasAnySignal) return null;

    return buildReadinessScore({
      cvScore: cvScoreInfo?.score,
      interviewScore: accountScores?.avgInterviewScore ?? undefined,
      technicalResult,
    });
  }, [cvScoreInfo, accountScores]);
  ...
  const voiceMinutesUsed = usage?.voiceMinutesUsed ?? 0;
  const voiceMinutesRemaining =
    usage?.voiceMinutesRemaining ??
    Math.max(0, limits.voiceMinutesPerMonth - voiceMinutesUsed);
    ...
    <main className="min-h-screen bg-canvas text-fg">
      ...
      {mobileOpen && (
        ...
        >
          ...
          <div
            className="relative h-full w-[260px] bg-canvas p-5"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setMobileOpen(false)}
              className="mb-4 grid h-10 w-10 place-items-center rounded-xl border border-line hover:bg-fg/5"
            >
              <X className="h-5 w-5" />
            </button>
            <Sidebar plan={plan} />
      ...
      <div className="px-5 pb-16 pt-20 lg:ml-[240px] lg:px-10 lg:pt-8">
        ...
        <header className="flex flex-wrap items-start justify-between gap-5 border-b border-line pb-7">
          <div>
            ...
            <p className="mt-2 text-sm text-muted">
              Track your progress and close the gap to your next offer.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <span
              className={cn(
                "rounded-full border px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em]",
                planBadgeStyle(plan),
              )}
            >
              {planLabel(plan)} plan
            ...
            <Link
              href="/onboarding"
              className="inline-flex items-center gap-2 rounded-2xl bg-brand px-5 py-3 text-sm font-black text-on-brand hover:bg-brand"
            >
        ...
        <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard
            ...
            sub={
              accountScores?.avgInterviewScore != null
                ? `Across ${accountScores.scoredInterviewCount} scored session${accountScores.scoredInterviewCount === 1 ? "" : "s"}`
                : isPremium
                  ? "Complete interviews to track"
          ...
          <MetricCard
            ...
            sub={
              readiness
                ? readiness.label
                : "Builds after your first interview or CV upload"
            }
            icon={Target}
            color="text-warning"
            locked={false}
          ...
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p
                  className={cn(
                    ...
                    isPremium ? "text-muted" : "text-muted",
                  )}
                >
              ...
              <Link
                href={
                  isPremium
                    ? "/pricing?plan=premium_pro"
                    : "/pricing?plan=premium"
                }
                className={cn(
                  "inline-flex shrink-0 items-center gap-2 rounded-xl px-5 py-3 text-sm font-black",
                  isPremium
        ...
        <section className="mt-8">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {actionCards.map((card) => {
              ...
              const dest = allowed
                ? card.href
                : `/pricing?intent=upgrade&plan=${card.required}`;
              return (
                <Link
                  key={card.title}
                  href={dest}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div
                      className={cn(
                        ...
                        allowed ? card.bg : "bg-fg/[0.04]",
                      )}
                    >
                      <Icon
                        className={cn(
                          "h-5 w-5",
                          allowed ? card.accent : "text-muted",
                        )}
                      />
                    </div>
                    {allowed ? (
                  ...
                  <p
                    className={cn(
                      ...
                      allowed ? "text-muted" : "text-muted",
                    )}
                  >
                    {allowed
                      ? card.detail
                      : `Unlock with ${WORKZO_PLAN_LIMITS[card.required].label}`}
                  </p>
                </Link>
              );
            })}
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-2xl border border-line bg-fg/[0.028] p-6">
            <div className="flex items-center justify-between gap-3">
              ...
              <Link
                href="/history"
                className="text-xs font-black text-brand hover:text-brand"
              >
                View all
              </Link>
            </div>
          ...
          <div className="rounded-2xl border border-line bg-fg/[0.028] p-6">
              ...
              {proTools.map((tool) => {
                ...
                return (
                  ...
                  >
                    ...
                    {allowed && (
                      <CheckCircle2 className="ml-auto h-4 w-4 shrink-0 text-brand" />
                    )}
                  </div>
                );
              })}
            ...
            {!isPro && (
              <Link
                ...
                className="mt-4 flex items-center justify-center gap-2 rounded-xl border border-brand/20 bg-brand/10 px-4 py-3 text-sm font-black text-brand hover:bg-brand/20"
              >
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted">
                ...
                structure every behavioural answer. Lead with the result first
                if your audience is senior. Recruiters remember the outcome, not
                the process.
              </p>
            </div>
          </div>
  ...
  useEffect(() => {
    try {
      const keys = Object.keys(localStorage).filter(
        (k) => k.startsWith("workzo") && k.includes("result"),
      );
        ...
        try {
          ...
          for (const item of arr) {
            if (item?.targetRole || item?.recruiterName) {
              found.push({
                id: item.id || k,
                targetRole: item.targetRole || "Interview",
                recruiterName: item.recruiterName || "AI Recruiter",
  ...
  if (sessions.length === 0) {
    return (
      <div className="mt-5 rounded-xl border border-line bg-fg/[0.02] p-6 text-center">
        ...
        <Link
          href="/onboarding"
          className="mt-4 inline-flex items-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-sm font-black text-on-brand hover:bg-brand"
        >
    ...
    <div className="mt-4 space-y-3">
      {sessions.map((s, idx) => (
        <Link
          key={`${s.id}-${idx}`}
          href="/results"
          className="group flex items-center justify-between gap-3 rounded-xl border border-line bg-fg/[0.02] p-3.5 transition hover:bg-fg/[0.05]"
        >
          <div className="flex items-center gap-3">
...
function Sidebar({ plan }: { plan: WorkZoPlanType }) {
  ...
  const premiumItems = new Set(["Job Assist", "Career Brain"]);
  ...
  return (
    <div className="flex h-full flex-col pb-4">
      ...
      <div
        className={cn(
          "mt-4 rounded-xl border px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em]",
          planBadgeStyle(plan),
        )}
      >
      ...
      <div className="mt-5 flex-1 overflow-y-auto pr-1 -mr-1">
        ...
        <nav className="space-y-0.5">
          {navItems.map((item) => {
            ...
            const href = locked
              ? `/pricing?intent=upgrade&feature=${item.label.toLowerCase().replace(/ /g, "_")}`
              : item.href;
            return (
              <Link
                key={item.label}
        ...
        {/* Pro nav items */}
        <div className="mt-5">
          <p className="px-3 text-[9px] font-black uppercase tracking-[0.24em] text-muted/60 mb-1.5">
            Premium Pro
          </p>
          <nav className="space-y-0.5">
            {proNavItems.map((item) => {
              ...
              const href = allowed ? item.href : "/pricing?plan=premium_pro";
              return (
                <Link
                  key={item.label}
                  href={href}
                  className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-bold transition ${
                    allowed
                      ...
                      : "text-muted/50 hover:bg-fg/[0.04]"
                  }`}
                >
                  <span className="flex items-center gap-3">
                    <Icon className="h-4 w-4" />
                  ...
                  {!allowed && <Lock className="h-3 w-3 text-muted/40" />}
                </Link>
              );
            })}
          </nav>
        ...
        {plan === "free" && (
          <div className="mt-6 rounded-2xl border border-brand/20 bg-brand/10 p-4">
            ...
            <p className="mt-1.5 text-xs leading-5 text-muted">
              120 AI voice minutes, Job Assist, Career Brain, advanced reports,
              and progress tracking.
            </p>
            <Link
              href="/pricing?plan=premium"
              className="mt-3 flex items-center justify-center gap-1.5 rounded-xl bg-brand px-3 py-2 text-xs font-black text-on-brand hover:bg-brand"
            >
              <Crown className="h-3.5 w-3.5" /> Upgrade:{" "}
              {displayPrices.premium.amount}/mo
            </Link>
          </div>
        )}
        ...
        {plan === "premium" && (
          <div className="mt-6 rounded-2xl border border-brand/20 bg-brand/10 p-4">
            ...
            <p className="mt-1.5 text-xs leading-5 text-muted">
              Upgrade to Premium Pro to unlock AI Career Coach, AI Video
              Interviews, roadmaps, and replay intelligence.
            </p>
            <Link
              href="/pricing?plan=premium_pro"
              className="mt-3 flex items-center justify-center gap-1.5 rounded-xl bg-brand px-3 py-2 text-xs font-black text-on-brand hover:bg-brand"
            >
              ...
              {displayPrices.premiumPro.amount}/mo
            </Link>
          </div>
        )}
      </div>
...
function MetricCard({
  ...
  const content = (
    <div
      className={cn(
        ...
        locked
          ? "border-line bg-fg/[0.02]"
          : "border-line bg-fg/[0.035] hover:bg-fg/[0.05]",
      )}
    >
  ...
  return locked && lockedHref ? (
    <Link href={lockedHref}>{content}</Link>
  ) : (
    <div>{content}</div>
  );
}
