...
import { getWorkZoCurrentPlan, getWorkZoDevPlanOverride, getWorkZoUsageSummary } from "@/lib/workzoUsageTracker";
import { readLatestInterviewSetup } from "@/lib/workzoInterviewSetup";
import { computeCvCompletenessScore, type WorkZoAccountScores } from "@/lib/workzoCvScore";
...
const proTools = [
  ...
  { title: "Live AI Recruiter", detail: "60 AI video minutes per month", icon: Video, feature: "video_recruiter" as const },
...
const proNavItems = [
  { label: "AI Career Coach", href: "/career-coach", icon: BrainCircuit, feature: "career_coach" as const },
  { label: "Career Roadmap", href: "/career-plan", icon: TrendingUp, feature: "career_roadmaps" as const },
  { label: "Replay Intelligence", href: "/replay", icon: PlayCircle, feature: "replay_intelligence" as const },
];
...
export default function DashboardPage() {
  ...
  const [accountScores, setAccountScores] = useState<WorkZoAccountScores | null>(null);
  const [cvScoreInfo, setCvScoreInfo] = useState<ReturnType<typeof computeCvCompletenessScore>>(null);
  useEffect(() => {
    let active = true;
    ...
    fetch("/api/account/plan", { cache: "no-store", credentials: "include" })
      ...
      .then((data) => {
        ...
        const serverPlan = getWorkZoDevPlanOverride() || normalizeWorkZoPlan(data.plan || p);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const profile = (setup?.resumeProfile as ResumeProfile | undefined) || null;
  ...
  const readiness = useMemo(() => {
    ...
    try {
      const raw = typeof window !== "undefined" ? window.localStorage.getItem("workzo_technical_result") : null;
      if (raw) technicalResult = JSON.parse(raw);
    } catch {}

    const hasAnySignal =
      Boolean(cvScoreInfo) || Boolean(accountScores?.avgInterviewScore) || Boolean(technicalResult);
  ...
  const voiceMinutesRemaining = usage?.voiceMinutesRemaining ?? Math.max(0, limits.voiceMinutesPerMonth - voiceMinutesUsed);
  const interviewsRemaining = usage?.interviewsRemaining ?? Math.max(0, limits.interviewsPerMonth - interviewsUsed);
    ...
    <main className="min-h-screen bg-canvas text-fg">
      ...
      <div className="px-5 pb-16 pt-20 lg:ml-[240px] lg:px-10 lg:pt-8">
        ...
        <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard
            label="Avg score"
            value={accountScores?.avgInterviewScore != null ? `${accountScores.avgInterviewScore}` : "—"}
            sub={
          ...
          <MetricCard
            label="CV score"
            value={!isPremium ? "Locked" : cvScoreInfo ? `${cvScoreInfo.score}` : "—"}
            sub={
              !isPremium
                ? "Upgrade to unlock"
                : cvScoreInfo
                  ? cvScoreInfo.label
                  : "Upload your CV to score"
            }
            icon={FileText}
            color="text-brand"
            locked={!isPremium}
          ...
          <MetricCard
            label="Readiness"
            value={!isPremium ? "Locked" : readiness ? `${readiness.overall}` : "—"}
            sub={
              !isPremium
                ? "Upgrade to unlock"
                : readiness
...
function Sidebar({ plan }: { plan: WorkZoPlanType }) {
  ...
  return (
    <div className="flex h-full flex-col pb-4">
      ...
      {/* Pro nav items */}
      <div className="mt-5">
        <p className="px-3 text-[9px] font-black uppercase tracking-[0.24em] text-muted/60 mb-1.5">Premium Pro</p>
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-bold transition ${
                  allowed ? "text-muted hover:bg-fg/[0.06] hover:text-fg" : "text-muted/50 hover:bg-fg/[0.04]"
                }`}
              >
                <span className="flex items-center gap-3">
                  <Icon className="h-4 w-4" />
      ...
      {plan === "premium" && (
        <div className="mt-6 rounded-2xl border border-brand/20 bg-brand/10 p-4">
          ...
          <p className="mt-1.5 text-xs leading-5 text-muted">Upgrade to Premium Pro to unlock AI Career Coach, AI Video Interviews, roadmaps, and replay intelligence.</p>
          <Link href="/pricing?plan=premium_pro" className="mt-3 flex items-center justify-center gap-1.5 rounded-xl bg-brand px-3 py-2 text-xs font-black text-on-brand hover:bg-brand">
            <Star className="h-3.5 w-3.5" /> Upgrade: {displayPrices.premiumPro.amount}/mo
          </Link>
        </div>
      )}
