...
export type CvIdentityConfirmProps = {
  ...
  onConfirm: (edited: { name: string; headline: string }) => void;
...
export default function CvIdentityConfirm({
  ...
  const initialHeadline = profile?.basics?.headline?.trim() || "";
  ...
  }, [open, initialName, initialHeadline]);

  const nameNeedsAttention = useMemo(() => isPlaceholder(name, PLACEHOLDER_NAMES), [name]);
  const headlineNeedsAttention = useMemo(
    () => isPlaceholder(headline, PLACEHOLDER_HEADLINES),
    [headline],
  );
  ...
  const confirm = () =>
    onConfirm({ name: name.trim(), headline: headline.trim() });
        ...
        <div className="mt-4 space-y-3.5">
          ...
          <label className="block">
            ...
            <input
              ...
              className={`${fieldClass} ${
                headlineNeedsAttention ? "border-brand/50" : "border-line"
              }`}
            />
          </label>
        </div>
