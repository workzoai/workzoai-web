  ...
  return (
    <section className="rounded-lg border border-violet-300/20 bg-violet-500/[0.08] p-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-violet-200">Premium Pro</p>
          <h2 className="mt-2 text-2xl font-black text-white">Unlock AI Career Coach + Replay Intelligence</h2>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 flex items-center gap-3 text-2xl font-black text-white">
            <BrainCircuit className="h-6 w-6 text-violet-200" /> AI Career Coach
          </h2>
          ...
          <div className="mt-4 space-y-3">
            {suite.replayIntelligence.slice(0, source === "dashboard" ? 3 : 6).map((moment) => (
              <div key={moment.id} className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                ...
                <p className="mt-2 text-xs font-bold text-slate-400">{moment.question}</p>
                <p className="mt-2 text-sm leading-6 text-slate-300">{moment.insight}</p>
                <p className="mt-3 rounded-xl bg-violet-400/10 p-3 text-xs leading-5 text-violet-50">Coach: {moment.coachingAction}</p>
              </div>
            ))}
          </div>
        </div>
