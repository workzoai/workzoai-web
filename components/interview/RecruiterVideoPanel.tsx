...
export default function RecruiterVideoPanel({ setup, statusLabel, statusTone = "LIVE", isMuted = false, onToggleMute, onOpenSettings, onEnd }: Props) {
  return (
    <section className="relative overflow-hidden rounded-xl border border-line bg-canvas shadow-2xl">
      <div className="relative min-h-[480px] w-full overflow-hidden sm:min-h-[540px] lg:min-h-[600px]">
        ...
        <div className="absolute left-6 top-6 rounded-xl border border-success/30 bg-success/10 px-4 py-3 text-sm font-black uppercase text-success">
          <span className="mr-2 inline-block h-2.5 w-2.5 rounded-full bg-success" />{statusTone}
        </div>
        <div className="absolute bottom-7 left-6">
          <div className="flex items-center gap-3"><h2 className="text-2xl font-black text-on-brand">{setup.recruiterName}</h2><span className="h-5 w-5 rounded-full bg-brand" /></div>
          <p className="mt-2 text-base text-fg">{setup.recruiterTitle}</p>
          <p className="mt-2 text-sm font-black text-success">{statusLabel}</p>
        </div>
